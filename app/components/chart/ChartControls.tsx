...
const IndicatorsDialog: React.FC<IndicatorsDialogProps> = ({
  ...
  const handleLengthChangeVWAP = (vwapId: string, length: number) => {
    const vwapConfig = vwapConfigs.find(vwap => vwap.id === vwapId);
    if (vwapConfig) {
      onUpdateVWAP(vwapId, { 
  ...
  const handleLineSizeChangeVWAP = (vwapId: string, lineSize: number) => {
    ...
    if (vwapConfig) {
      onUpdateVWAP(vwapId, { 
        lineSize: Math.max(0.5, Math.min(5, lineSize)) 
      });
    }
  };
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      {vwapConfigs.map((vwapConfig) => (
                        <div key={vwapConfig.id} className="bg-gray-750 rounded-lg border border-gray-700 p-6">
                          {/* Main VWAP Toggle */}
                          <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-3">
                              <input
                                ...
                                className="w-4 h-4 rounded border-gray-600 bg-gray-700 text-yellow-500 focus:ring-yellow-500 focus:ring-2"
                              />
                              <h4 className="text-md font-medium text-white">VWAP ({vwapConfig.length})</h4>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                              ...
                              <input
                                ...
                                onChange={(e) => handleLengthChangeVWAP(vwapConfig.id, parseInt(e.target.value) || 20)}
                                className="w-full bg-gray-700 border border-gray-600 rounded px-3 py-2 text-white text-sm focus:outline-none focus:ring-1 focus:ring-yellow-500"
                                title="Number of periods for VWAP calculation"
                              />
                              <div className="text-xs text-gray-400 mt-1">
                                {vwapConfig.length === 0 ? 'Session-based' : `${vwapConfig.length} periods`}
                              </div>
                            </div>

                            {/* Line Width */}
                            <div>
                              <label className="block text-sm text-gray-400 mb-2">Line Width</label>
                              <input
                                type="number"
                                min="0.5"
                                max="5"
                          ...
                          <div className="mt-6 p-4 bg-gray-800 rounded border border-gray-700">
                            <h5 className="text-sm font-medium text-gray-300 mb-2">VWAP Information</h5>
                            <div className="text-xs text-gray-400 space-y-1">
                              <p><strong>Length {vwapConfig.length}:</strong> {vwapConfig.length === 0 ? 
                                'Session-based VWAP (resets at UTC midnight)' : 
                                `Rolling VWAP over ${vwapConfig.length} periods`}
                              </p>
                              <p><strong>Calculation:</strong> Σ(Price × Volume) / Σ(Volume)</p>
                              <p><strong>Price used:</strong> Typical Price = (High + Low + Close) / 3</p>
                            </div>
                          </div>
                        </div>
                      ))}
