  ...
  <div className="space-y-4">
    <div className="flex items-center justify-between">
      ...
      <div className="text-sm text-gray-400">
        {configs.filter(config => config.show).length} active
      </div>
    </div>
    ...
    <div className="space-y-3">
      {configs.map((config) => (
        <div key={config.id} className="flex items-center gap-4 p-3 bg-gray-750 rounded-lg border border-gray-600">
          ...
          <span className="text-sm text-white font-medium min-w-[80px]">TRIX</span>
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              max="200"
              value={config.period}
              onChange={(e) => onPeriodChange(config.id, parseInt(e.target.value) || 14)}
