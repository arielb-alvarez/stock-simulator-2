...
const useWebsocketsTradingTrades = ({ parameters, wsRef }: Props) => {
    const WS_TRADING_TRADES_URL: string | undefined = process.env.NEXT_PUBLIC_USER_BROADCASTING;
    const WS_CONNECT = () => {
        try {
            if (!WS_TRADING_TRADES_URL) {
            ...
            const channel = `${EUserBroadcastingChannel.TRADE}:${transformedSymbol}:${interval}`.toLowerCase();
            ...
            const ws = new WebSocket(WS_TRADING_TRADES_URL);
            ws.binaryType = 'arraybuffer';
            wsRef.current = ws;

            ws.onopen = () => {
                try {
                    console.log("[WebSocket] Connected to trade");
...
export default useWebsocketsTradingTrades;
