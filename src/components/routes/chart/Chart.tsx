  ...
  useEffect(() => {
    ...
    const initChart = async () => {
      ...
      try {
        ...
        const tradingCandlesData: TDataTradingCandles[] = candlestickData.map(item => ({
          timestamp: item.time,
          open: item.open,
          high: item.high,
          low: item.low,
        ...
        // Update data reference with converted data
        currentDataRef.current = tradingCandlesData;

        // Update chart with data
        updateChartWithData(chartInstance, tradingCandlesData, false);
